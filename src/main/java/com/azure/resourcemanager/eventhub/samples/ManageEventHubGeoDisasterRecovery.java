 ...
 * license information.
 */
package com.azure.resourcemanager.eventhub.samples;
...
/**
 * Azure Event Hub sample for managing geo disaster recovery pairing - - Create
 * two event hub namespaces - Create a pairing between two namespaces - Create
 * an event hub in the primary namespace and retrieve it from the secondary
 * namespace - Retrieve the pairing connection string - Fail over so that
 * secondary namespace become primary.
 */
    ...
    public static boolean runSample(AzureResourceManager azureResourceManager) {
        ...
        try {
            ...
            ResourceGroup resourceGroup = azureResourceManager.resourceGroups().define(rgName)
                    .withRegion(Region.US_SOUTH_CENTRAL).create();

            System.out.println("Creating primary event hub namespace " + primaryNamespaceName);

            EventHubNamespace primaryNamespace = azureResourceManager.eventHubNamespaces().define(primaryNamespaceName)
                    .withRegion(Region.US_SOUTH_CENTRAL).withExistingResourceGroup(resourceGroup).create();
            ...
            EventHubNamespace secondaryNamespace = azureResourceManager.eventHubNamespaces()
                    .define(secondaryNamespaceName).withRegion(Region.US_NORTH_CENTRAL)
            ...
            pairing = azureResourceManager.eventHubDisasterRecoveryPairings().define(geoDRName)
                    .withExistingPrimaryNamespace(primaryNamespace).withExistingSecondaryNamespace(secondaryNamespace)
                    .create();
            ...
            EventHub eventHubInPrimaryNamespace = azureResourceManager.eventHubs().define(eventHubName)
                    .withExistingNamespace(primaryNamespace).withNewConsumerGroup("consumerGrp1").create();
            ...
            EventHub eventHubInSecondaryNamespace = azureResourceManager.eventHubs().getByName(rgName,
                    secondaryNamespaceName, eventHubName);
